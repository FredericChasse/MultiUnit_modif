...
Jsingle     = zeros(nIterations, nParticles, 2);
...
for iData = 1 : nIterations
  ...
  for iUnit = 1 : nParticles
    ...
    J(iData, iUnit) = Jsingle(iData, iUnit, 1) + Jsingle(iData, iUnit, 2);
...
%% Simulation 2D
gamma = mean(Jsingle(end,:,1)-Jsingle(end,:,2));
beta  = mean(d(end,:,1)-d(end,:,2));
...
%   end
  for iUnit = 1 : nParticles
    ...
    if (iUnit > 4)
      ...
      J(iData, iUnit) = -0.002 * (d(iData, iUnit, 1) - 110 - beta)^2 + 19 + gamma;
    end
  end
  %========================================================================
