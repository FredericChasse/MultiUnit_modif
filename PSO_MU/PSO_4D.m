...
% Simulation data
d           = zeros(nIterations, nParticles, 3);
J           = zeros(nIterations, nParticles);
Jsingle     = zeros(nIterations, nParticles, 3);
MaxVal      = zeros(nIterations, nParticles);
Pbest       = zeros(nIterations, nParticles, 3);
v           = zeros(nIterations, nParticles, 3);

MaxOfUnits  = zeros(nIterations, 1);
Gbest       = zeros(nIterations, 3);
...
for iUnit = 1 : nParticles
  ...
  d(1, iUnit,3) = rand* (dmax-dmin) + dmin;
...
for iData = 1 : nIterations
  ...
  %========================================================================
  for iUnit = 1 : nParticles
    Jsingle(iData, iUnit, 1) = -0.002 * (d(iData, iUnit, 1) - 100)^2 + 20;
    Jsingle(iData, iUnit, 2) = -0.002 * (d(iData, iUnit, 2) - 110)^2 + 19;
    Jsingle(iData, iUnit, 3) = -0.002 * (d(iData, iUnit, 3) - 95 )^2 + 22;
